  ...
  dp.sh.HighlightAll = function (
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      highlighter.addControls =
        showControls == null
          ? !IsOptionSet("nocontrols", options)
          : showControls;
...
function DPH_AddCopyBtn(dph) {
  ...
  d.style.cssText =
    "position:absolute;right:15px;top:5px;background-color:rgba(66,66,66,0.6);color:#fff;border-radius:5px;cursor:pointer;padding:1px 6px";
  d.innerText = "copy";
  dph.appendChild(d);
...
function ReplaceDPH(dph) {
  ...
  if (text.substring(0, 8) != "<script>") return;
  if (text.substring(text.length - 8, text.length) != "/script>") return;
